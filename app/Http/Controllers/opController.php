...
class opController extends Controller
{
  ...
  {
    $klienkami = klienkami::orderBy('created_at', 'desc')->paginate(10);
    return view('operator.klienkami.all', ['klienkami' => $klienkami]);
  }

  public function buat_klienkami()
  {
    return view('operator.klienkami.buat');
  }

  public function save_klienkami(Request $request){
    $klienkami = new klienkami;
    $klienkami->judul = $request->judul;
    $klienkami->url = $request->url;
    $klienkami->photo = '';
    if($request->hasFile('photo')){
      $photo = date('YmdHis').uniqid().".".
      $request->photo->getClientOriginalExtension();
      $request->photo->move(public_path()."/assets/klienkami",$photo);
      $klienkami->photo = $photo;
    }
    $klienkami->save();
    return redirect(url('operator/klienkami'));
  }

  public function klienkami_edit($id){
    $klienkami = klienkami::find($id);
    return view('operator.klienkami.edit', ['klienkami' => $klienkami]);
  }
  ...
  public function klien_update(Request $request, $id){
    ...
    {
      $klienkami->photo = $klienkami->photo;
    } else{
      $klienkami->photo = '';
      if($request->hasFile('photo')){
        $photo = date('YmdHis').uniqid().".".
    ...
    $klienkami->save();
    return redirect('operator/klienkami');
  }
